...
### 5. Tauri sidecar の `_child` を捨てると Go プロセスが残り続ける

Tauri v2 の `tauri_plugin_shell::Command::spawn()` は `(rx, child)` を返すが、
`CommandChild` は **Drop で子プロセスを kill しない仕様**。よくある書き方:

```rust
let (mut rx, _child) = sidecar_command.spawn().expect(...);
```

`_child` を捨てるとフロントウィンドウを閉じても Go バックエンドが残り続け、
何度も起動・終了するうちに同名プロセスが taskmgr に大量に並ぶ。SQLite を
ロックしたままになる事故もある。

本テンプレートでは下記 2 段構えで対処済み (`src-tauri/src/lib.rs` +
`backend/main.go`):

1. **Tauri 側**: `CommandChild` を `AppState` に保管し、`RunEvent::ExitRequested`
   で `child.kill()` を呼ぶ。`.run(callback)` 形式に分割するのがポイント。
2. **Go 側**: `monitorParentStdin()` ゴルーチンで `os.Stdin.Read` を回し、
   親死亡で stdin が EOF になったら `os.Exit(0)`。これで `taskkill /F` /
   Activity Monitor の強制終了経路でも子がゾンビ化しない。

派生アプリでこの 2 つを **両方** コピペするか、最低でもどちらかは入れること。
`isStdinTerminal()` で開発時 (terminal 直起動) は監視オフにしている。

### 6. Cargo / webview2-com / windows-rs のバージョン整合
